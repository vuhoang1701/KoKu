...
class NetworkManager {
    ...
    static func updateTransaction(transaction: TransactionModel ,success: @escaping () -> Void, failure: @escaping (_ error: NSError) -> Void)
    {
        
        let transactionURL = baseURL + "transactions/\(transaction.id!)"
        var parameters = ["fromCurrency":transaction.fromCurrency,
        ...
        if let receivedTime = transaction.receivedTime
        {
            parameters["receivedTime"] = receivedTime
        }
        
        self.request(transactionURL, method: .put, parameters: parameters , success: { (jsonData) in
            if let jsonDictionary = jsonData as? [NSObject : Any] as NSDictionary?
            {
